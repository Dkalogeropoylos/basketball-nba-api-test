...
st.title("🏀 NBA Pricing Engine — Phase B sandbox (V2.18.2 framework)")
st.caption(
    "NBA-only sandbox built from the frozen V2.18.2 + DREB framework. "
    "SportsDataverse NBA data are normalized to the same schema; NBA-specific "
    "calibrations are learned from NBA history. WNBA production is untouched."
)
...
with st.sidebar:
    ...
    st.write("NBA historical:", "✅ SportsDataverse GitHub releases")
    st.write("stats.nba.com:", "🚫 not used")
    st.write("NBA minutes constraint:", "✅ 240 regulation minutes")
    st.write("NBA regulation:", "✅ 48 minutes")
    st.divider()
    ...
    season = st.number_input(
        "Season end year (2026 = 2025-26)", min_value=2002, max_value=2100, value=2026, step=1
    )


@st.cache_data(ttl=21600, show_spinner=False)
def load_sportsdataverse_season(season):
    return SportsDataverseNBA(timeout=45).load_season(int(season))


@st.cache_data(ttl=3600, show_spinner=False)
def cached_advanced_player(season, player_id, measure):
    provider, _ = get_advanced_provider("NBA", bdl_key)
    if provider is None:
        return []
...
with tab_game:
    ...
    if data_pack is None:
        ...
        if st.button(f"Load NBA {int(season)} database", type="primary"):
            try:
                with st.spinner("Loading NBA historical database..."):
    ...
    if setup and pace:
        ...
        st.caption(
            ...
            "Fast/slow control weights are fitted from completed NBA games; "
    ...
    if setup and data_pack is not None:
        ...
        with st.expander("Shared minute overrides / restrictions", expanded=False):
            st.caption(
                ...
                "the full 240-minute rotation, Team Markets and Player Props."
            )
            ...
            if away_min_names:
                ...
                for i, name in enumerate(away_min_names):
                    ...
                    away_values[name] = cols[i % len(cols)].number_input(
                        f"{name} expected MIN", min_value=0.0, max_value=48.0,
                        value=float(np.clip(default, 0.0, 48.0)), step=0.5,
...
with tab_team:
    ...
    if data_pack is None:
        ...
        # player just left the roster), a synthetic 240-minute counterfactual
        ...
        with st.expander("Automatic matchup/location modifiers — optional trader override", expanded=False):
            st.caption(
                ...
                "walk-forward NBA history using disjoint Old/G6-10/L5 + opponent + H2H inputs. FGA remains a "
                "possession-identity consequence. NBA official AST are tied to made field goals; free-throw "
                "assist credit is a FIBA/EuroLeague statistical rule and is not mixed into NBA markets."
            )
        ...
        if pack and pack.get("fingerprint") == fingerprint:
            ...
            with st.expander("Model audit: buckets / location / H2H / conservation"):
                ...
                st.caption(
                    "Healthy / OUT-only / current are 240-minute synthetic counterfactuals. The OUT bridge fades separately by stat as "
...
with tab_player:
    ...
    if data_pack is None:
        ...
        # team stays at 240 regulation minutes.
        ...
        if selected_names:
            ...
            st.caption(
                ...
                "downweighting, non-overlapping buckets and a 240-minute team "
            ...
            with st.expander("Full rotation minute audit"):
                st.caption(
                    "Each team is constrained to 240 regulation minutes. "
            ...
            same_role_enabled = st.toggle(
                ...
                help=(
                    ...
                    "reused as 3/5 or 2/5. Position/role relevance changes by stat, while minutes remain separate in the 240-minute engine."
                ),
            ...
            if st.button(
                ...
                for pname in selected_names:
                    ...
                    #   1) 240-minute engine -> MINUTES after confirmed OUTs
...
- A 240-minute roster counterfactual supplies only residual fallback, fading separately by stat as near-state confidence rises.
...
**Minutes / pace**
- Full team rotation is constrained to 240 regulation minutes.
...
- Pace control is fitted from completed NBA games with a mild ridge prior.
