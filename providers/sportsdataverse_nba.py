...
class SportsDataverseNBA:
    """
    NBA historical provider backed by SportsDataverse GitHub release assets.

    Core model needs only two season files:
      - ESPN NBA player boxscores
      - ESPN NBA team boxscores
    ...
    def load_season(self, season: int) -> Dict[str, object]:
        ...
        # Keep the 30 NBA franchises only. SportsDataverse/ESPN season_type=2
        # also contains All-Star mini-games under special team IDs.
        nba_team_ids = set(range(1, 31))
        if "TEAM_ID" in team.columns:
            team = team[pd.to_numeric(team["TEAM_ID"], errors="coerce").isin(nba_team_ids)].copy()
        if "TEAM_ID" in player.columns:
            player = player[pd.to_numeric(player["TEAM_ID"], errors="coerce").isin(nba_team_ids)].copy()

        # NBA Cup championship is an extra 83rd game for its two finalists and
        # is tagged as season_type=2 by ESPN. For historical regular-season
        # calibration, remove only a game that creates exactly two 83-game teams.
        counts = team.groupby("TEAM_ABBR")["GAME_ID"].nunique()
        over82 = counts[counts > 82].index.astype(str).tolist()
        if len(over82) == 2:
            pair = team[
                team["TEAM_ABBR"].astype(str).isin(over82)
                & team["OPP_ABBR"].astype(str).isin(over82)
            ].copy()
            pair_dates = pair.groupby("GAME_ID")["GAME_DATE"].min().sort_values()
            # The extra Cup final precedes the ordinary later-season H2Hs;
            # removing the earliest pair game restores 82 games per finalist.
            for gid in pair_dates.index.astype(str):
                test_team = team[team["GAME_ID"].astype(str) != gid]
                tc = test_team.groupby("TEAM_ABBR")["GAME_ID"].nunique()
                if tc.get(over82[0], 0) == 82 and tc.get(over82[1], 0) == 82:
                    team = test_team.copy()
                    player = player[player["GAME_ID"].astype(str) != gid].copy()
                    break
        ...
        # player minutes sum to 240 in regulation, 265 in 1OT, 290 in 2OT, etc.
        ...
        _ot_count = np.maximum(np.rint((_game_len - 48.0) / 5.0), 0).astype(int)
        _game_length = 48.0 + 5.0 * _ot_count
        ...
        _legacy_ot = set(
            player.loc[pd.to_numeric(player["MIN"], errors="coerce") > 48, "GAME_ID"].astype(str)
        )
        for _gid in _legacy_ot:
            if float(_length_map.get(_gid, 48.0)) <= 48.0:
                _length_map[_gid] = 53.0
                _ot_map[_gid] = 1

        for _df in (player, team):
            _gid = _df["GAME_ID"].astype(str)
            _df["GAME_LENGTH_MIN"] = _gid.map(_length_map).fillna(48.0).astype(float)
    ...
    def position_environment(
        ...
        return (
            SportsDataverseNBA._per36(vs),
            SportsDataverseNBA._per36(x),
        )
